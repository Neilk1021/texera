...
object AppleAuthResource {
  ...
  /**
    ...
    * does for a Google account with no name. Apple supplies no avatar at all, hence `None`,
    * which the provisioner treats as "leave whatever is stored alone".
    */
...
class AppleAuthResource extends LazyLogging {
  ...
  def login(credential: String): TokenIssueResponse =
    verifiedClaims(credential) match {
      case Some(claims) =>
        val profile = AppleAuthResource.profileOf(claims)
        if (profile.email.isEmpty) {
          logger.warn(
            s"Refusing Apple identity ${profile.providerId}: the token carries no email address."
          )
